...
- `DemoDataSeeder` cria dois usuários fixos no boot (`admin_demo@oficina.demo` / `admin123`, role `ADMIN`; `visitante@oficina.demo` / `visitante123`, role `CLIENTE`) e gera uma oficina em escala real (~44 clientes, ~78 veículos, ~218 OS ao longo de 8 meses) na subida da aplicação e a cada 6h (`@Scheduled`, `initialDelay` pra não duplicar com o reset do boot). CPF/CNPJ são gerados com dígito verificador válido de verdade (`GeradorDocumento`) porque o Hibernate valida `@CPF`/`@CNPJ` automaticamente ao salvar, mesmo fora do fluxo normal da API.
- A massa de dados vem do `CatalogoDemo` (nomes, cidades, modelos de carro, catálogo de serviços com faixa de preço) e é montada com um `Random` de **semente fixa** (`SEMENTE`): cada reset recria exatamente a mesma oficina, com os mesmos números — o portfólio não muda de cara a cada 6h. Meses passados só têm OS finalizada/cancelada; o mês corrente é o único com OS aberta ou em andamento. Para isso funcionar, `OrdemServico.prePersist()` e `Cliente.prePersist()` só preenchem a data quando ela vem nula — antes forçavam `LocalDate.now()`, o que impedia qualquer histórico.
